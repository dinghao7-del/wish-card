...
const PRESCHOOL_INTERESTS = [
  '体能游戏', '游泳启蒙', '创意美术', '音乐律动', '绘本阅读',
  '乐高拼搭', '自然观察', '平衡车', '亲子运动', '表达小剧场',
  '生活自理小游戏', '手工黏土',
];

const LOWER_PRIMARY_INTERESTS = [
  '跳绳', '游泳', '篮球启蒙', '足球启蒙', '武术/跆拳道',
  '绘画', '钢琴启蒙', '合唱/声乐', '硬笔启蒙', '围棋',
  '乐高/机器人', '英语听说', '演讲口才', '科学实验',
];

const GENERAL_BOY_INTERESTS = [
...
const STAGES: Record<ScheduleStageKey, ScheduleOptimizationStage> = {
  preschool: {
    ...
    ageRange: '3-6岁',
    sleepTarget: '10-13小时',
    coreFocus: ['规律作息', '生活自理', '语言表达', '同伴交往', '户外运动', '游戏化探索'],
  },
  lower_primary: {
    key: 'lower_primary',
    label: '小学低年级',
    ...
    ageRange: '6-8岁',
    sleepTarget: '10小时左右',
    coreFocus: ['学习习惯', '阅读书写', '计算基础', '整理能力', '运动习惯'],
  },
  middle_primary: {
    key: 'middle_primary',
    label: '小学中年级',
    ...
    ageRange: '9-10岁',
    sleepTarget: '9-10小时',
    coreFocus: ['独立作业', '错题复盘', '阅读写作', '兴趣筛选', '运动坚持'],
  },
  upper_primary: {
    key: 'upper_primary',
    label: '小学高年级',
    ...
    ageRange: '11-12岁',
    sleepTarget: '9小时左右',
    coreFocus: ['自主学习', '小升初衔接', '时间管理', '兴趣聚焦', '抗压能力'],
  },
  junior_middle: {
    key: 'junior_middle',
    label: '初中阶段',
    ...
    ageRange: '12-15岁',
    sleepTarget: '8-9小时',
    coreFocus: ['多科平衡', '弱科突破', '错题体系', '运动减压', '考试节奏'],
  },
  senior_high: {
    key: 'senior_high',
    label: '高中阶段',
    ...
    ageRange: '15-18岁',
    sleepTarget: '8小时左右',
    coreFocus: ['升学目标', '选科策略', '高效复盘', '压力管理', '体能兜底'],
  },
};
...
function getSubjectOptions(stage: ScheduleOptimizationStage): string[] {
  switch (stage.key) {
    ...
    case 'middle_primary':
      return MIDDLE_PRIMARY_SUBJECTS;
    case 'upper_primary':
...
function getForbiddenSubjects(stage: ScheduleOptimizationStage): string[] {
  ...
  if (stage.key === 'junior_middle') return [];
  return [];
}

function isAllowedSubject(stage: ScheduleOptimizationStage, subject: string): boolean {
  const options = getSubjectOptions(stage);
  if (options.includes(subject)) return true;
  return !getForbiddenSubjects(stage).includes(subject);
}

function getStageInterestOptions(stage: ScheduleOptimizationStage) {
  if (stage.key === 'preschool') {
    return {
      all: PRESCHOOL_INTERESTS,
      boy: PRESCHOOL_INTERESTS,
      girl: PRESCHOOL_INTERESTS,
    };
  }
  if (stage.key === 'lower_primary') {
    return {
      all: LOWER_PRIMARY_INTERESTS,
      boy: uniq([...LOWER_PRIMARY_INTERESTS, '篮球启蒙', '足球启蒙']),
      girl: uniq([...LOWER_PRIMARY_INTERESTS, '中国舞启蒙', '创意美术']),
    };
  }
  return {
    all: uniq([...GENERAL_BOY_INTERESTS, ...GENERAL_GIRL_INTERESTS]),
    boy: GENERAL_BOY_INTERESTS,
    girl: GENERAL_GIRL_INTERESTS,
  };
}

function getParentExpectationOptions(stage: ScheduleOptimizationStage): string[] {
  if (stage.key === 'preschool') {
    return ['规律作息', '生活自理', '语言表达', '同伴交往', '户外运动', '情绪稳定', '亲子陪伴', '兴趣启蒙'];
  }
  if (stage.key === 'lower_primary') {
    return ['学习习惯', '阅读书写', '计算基础', '整理能力', '运动习惯', '自信表达', '减少磨蹭', '兴趣探索'];
  }
  if (stage.key === 'middle_primary' || stage.key === 'upper_primary') {
    return ['提升学习效率', '查漏补缺', '阅读写作', '数学思维', '兴趣聚焦', '运动健康', '自主安排', '小升初衔接'];
  }
...
function buildAiRules(stage: ScheduleOptimizationStage): string[] {
  const base = [
    `先判断孩子处于${stage.label}，所有问题、日程、商业推荐都必须服从这个年龄阶段。`,
    `输出要千人千面：结合年龄、年级、城市、学校类型、已有课外班、家庭预算、性格、屏幕时间和家长期望，不套固定模板。`,
    `日程建议必须先保障睡眠、户外/运动、亲子沟通和可执行性，再谈学习效率。`,
    `推荐教育产品时只使用行为标签和场景需求，不暴露家庭隐私原文。`,
  ];

  if (stage.key === 'preschool') {
    return [
      ...base,
      '禁止把幼儿园孩子包装成学科补习模型；不能出现物理、化学、历史、地理、政治、生物等学科强化建议。',
      '幼儿园阶段重点是健康、语言、社会、科学探究、艺术感受五类发展，但表达上要变成生活化问题。',
      '推荐以体能游戏、绘本共读、生活自理、表达小剧场、自然观察、创意美术为主，不建议题海、提前刷题、长时间端坐。',
      '每天结构应以幼儿园/家庭作息、户外活动、晚饭后安静亲子活动和睡前流程为主。',
    ];
  }
  ...
  if (stage.key === 'lower_primary') {
    return [
      ...base,
      '小学低年级只做基础学习能力和习惯建模，避免初高中学科提前化。',
      '用阅读表达、书写习惯、计算基础、英语听说、专注完成等能力替代笼统的强弱科目。',
      '作业安排要短时分段，避免把所有空闲时间都填成学习。',
    ];
  }
  ...
  if (stage.key === 'middle_primary' || stage.key === 'upper_primary') {
    return [
      ...base,
      '小学中高年级可以讨论语数英科学，但不能引入物理化学等初中学科。',
      '重点从陪伴完成转为独立完成、错题复盘、阅读写作和兴趣聚焦。',
    ];
  }

  return [
    ...base,
    '初高中可以进入多学科时间管理，但仍要结合具体年级，避免把所有孩子都推向同一种补课方案。',
    '对薄弱科目给出短周期复盘和诊断建议，商业推荐应围绕具体弱项、考试节点、心理压力和时间冲突。',
  ];
}

function buildSourceNotes(stage: ScheduleOptimizationStage): string[] {
  if (stage.key === 'preschool') {
    return [
      '参考3-6岁儿童学习与发展指南：健康、语言、社会、科学、艺术是幼儿发展主线。',
      '参考知乎/小红书类家长社区经验：幼儿阶段日程优化优先解决睡眠、户外、生活自理、亲子阅读、情绪稳定和低压兴趣体验。',
    ];
  }
  if (stage.key === 'lower_primary') {
    return [
...
function buildCommercialAngles(stage: ScheduleOptimizationStage): string[] {
  if (stage.key === 'preschool') {
    return ['体能/感统', '绘本阅读', '创意美术', '生活自理工具', '亲子活动', '自然探索'];
  }
  if (stage.key === 'lower_primary') {
    return ['阅读分级', '书写习惯', '计算基础', '运动启蒙', '英语听说', '学习桌面管理'];
  }
  if (stage.key === 'middle_primary' || stage.key === 'upper_primary') {
    return ['错题复盘', '阅读写作', '数学思维', '科学实验', '运动训练', '小升初规划'];
  }
...
export function sanitizeChildProfileForScheduleStage(profile: ChildProfile): ChildProfile {
  ...
  return {
    ...profile,
    strongSubjects: filter(profile.strongSubjects),
    weakSubjects: filter(profile.weakSubjects),
  };
}

export function buildScheduleOptimizationSkill(profile: ChildProfile): ScheduleOptimizationSkill {
  const stage = inferScheduleOptimizationStage(profile);
  const sanitizedProfile = sanitizeChildProfileForScheduleStage(profile);
  const interestOptions = getStageInterestOptions(stage);
  ...
  return {
    ...
    academicStepTitle: isPreschool
      ? '发展重点与陪伴时间'
      : isLowerPrimary
        ? '学习习惯与时间'
        : '学业与时间',
    academicStepDescription: isPreschool
      ? '了解孩子现在的生活能力、表达、运动和亲子陪伴时间。'
      : isLowerPrimary
        ? '先看基础学习习惯、作业节奏和放学后的可用时间。'
        : '了解当前学科状态、作业时长和可调整的课后时间。',
    ...
    sourceNotes: buildSourceNotes(stage),
    commercialRecommendationAngles: buildCommercialAngles(stage),
  };
}
...
export function buildScheduleOptimizationSkillPrompt(profile: ChildProfile): string {
  ...
  return [
    ...
    `孩子阶段：${skill.stage.label}（${skill.stage.ageRange}）`,
...
export function sanitizeScheduleRecommendationForStage(
  recommendation: ScheduleRecommendation,
  profile: ChildProfile,
): ScheduleRecommendation {
  const skill = buildScheduleOptimizationSkill(profile);
  const subjectAdvice: SubjectAdvice[] = recommendation.subjectAdvice
    .filter(advice => isAllowedSubject(skill.stage, advice.subject))
    .map(advice => ({
      ...advice,
      resources: advice.resources.filter(resource => !getForbiddenSubjects(skill.stage).some(subject => resource.includes(subject))),
    }));

  const avoidActivities = uniq([
    ...recommendation.avoidActivities,
    ...(skill.stage.key === 'preschool' ? ['提前学科补习', '长时间刷题', '睡前使用屏幕'] : []),
    ...(skill.stage.key === 'lower_primary' ? ['提前学习初高中学科', '用刷题替代阅读和运动'] : []),
  ]);
