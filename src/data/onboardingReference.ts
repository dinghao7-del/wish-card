...
export const kindergartenSchedule = {
  ...
  ageSpecificTasks: {
    '3-4': {
      focus: '基础自理能力',
      tasks: ['自己吃饭', '自己上厕所', '收拾玩具', '主动问好'],
    },
    '4-5': {
      focus: '社交与规则',
      tasks: ['自己穿衣服', '自己刷牙', '分享玩具', '整理书包'],
    },
    '5-6': {
      focus: '入学准备',
      tasks: ['自己整理物品', '按时完成作业', '阅读绘本', '写字练习'],
    },
  },
};
...
export const primarySchedule = {
  ...
  gradeSpecific: {
    '1-2': {
      focus: '适应小学、培养习惯',
      tasks: ['按时完成作业', '整理书包', '阅读绘本', '按时起床睡觉'],
      studyTime: '每天学习1-1.5小时',
    },
    '3-4': {
      focus: '巩固基础、拓展阅读',
      tasks: ['预习复习', '阅读30分钟', '练字', '户外运动'],
      studyTime: '每天学习1.5-2小时',
    },
    '5-6': {
      focus: '小升初准备、自主学习',
      tasks: ['自主完成作业', '系统复习', '错题整理', '时间管理'],
      studyTime: '每天学习2-2.5小时',
    },
  },
};
...
export const familyTypeRecommendations = {
  nuclear: {
    name: '核心家庭',
    description: '父母+孩子',
    characteristics: '关注度高，时间相对充裕',
    focusAreas: ['亲子陪伴', '习惯养成', '全面发展'],
  },
  extended: {
    name: '大家庭',
    description: '三代同堂或多子女',
    characteristics: '需要协调多代人教育理念',
    focusAreas: ['代际沟通', '兄弟姐妹关系', '独立能力'],
  },
  singleParent: {
    name: '单亲家庭',
    description: '单亲抚养',
    characteristics: '时间精力有限，需要高效管理',
    focusAreas: ['时间管理', '孩子独立性', '情感支持'],
  },
};
...
export interface FamilyProfile {
  familyType: 'nuclear' | 'extended' | 'singleParent';
  hasChildren: boolean;
 ...
 * 根据家庭画像生成智能推荐
 */
export function generateRecommendations(profile: FamilyProfile) {
  const recommendations: {
    tasks: Array<{ name: string; category: string; reason: string; points: number }>;
    habits: Array<{ name: string; frequency: string; reason: string }>;
    activities: Array<{ name: string; type: string; reason: string }>;
  ...
  profile.children.forEach((child) => {
    ...
    const ageSchedule = getScheduleByAgeGroup(ageGroup);
    const ageTasks = getTasksByAgeGroup(ageGroup, child);

    // 添加任务推荐
    ageTasks.forEach((task) => {
      const priority = profile.priorities[0];
      const multiplier = priorityWeights[priority]?.taskMultiplier[task.category] || 1;
      
      recommendations.tasks.push({
        name: task.name,
        category: task.category,
        reason: generateReason(task, child, profile),
        points: Math.round(task.points * multiplier),
      });
    });

    // 添加习惯推荐
    recommendations.habits.push(...generateHabits(ageGroup, child, profile));

    // 添加活动推荐
    if (profile.hasExtracurricular) {
      recommendations.activities.push(...generateActivities(child, profile));
    }
  });

  // 去重
  recommendations.tasks = deduplicateByName(recommendations.tasks);
  recommendations.habits = deduplicateByName(recommendations.habits);
  recommendations.activities = deduplicateByName(recommendations.activities);
...
function getScheduleByAgeGroup(ageGroup: AgeGroup) {
  switch (ageGroup) {
    case 'kindergarten':
      return kindergartenSchedule.dailyRoutine;
    case 'primary':
      return primarySchedule.dailyRoutine;
    case 'middle':
      return middleSchoolSchedule.dailyRoutine;
    case 'high':
      return highSchoolSchedule.dailyRoutine;
  }
}

function getTasksByAgeGroup(ageGroup: AgeGroup, child: ChildInfo) {
  let tasks: Array<{ name: string; category: string; points: number }> = [];

  switch (ageGroup) {
    case 'kindergarten': {
      const kg = kindergartenSchedule;
      kg.recommendedTasks.forEach((cat) => {
        cat.tasks.forEach((t) => {
          tasks.push({ name: t.name, category: cat.category, points: t.points });
        });
      });
      break;
    }
    case 'primary': {
      const pri = primarySchedule;
      pri.recommendedTasks.forEach((cat) => {
        cat.tasks.forEach((t) => {
          tasks.push({ name: t.name, category: cat.category, points: t.points });
        });
      });
    ...
    case 'middle': {
      const mid = middleSchoolSchedule;
      mid.recommendedTasks.forEach((cat) => {
        cat.tasks.forEach((t) => {
          tasks.push({ name: t.name, category: cat.category, points: t.points });
        });
      });
    ...
    case 'high': {
      const high = highSchoolSchedule;
      high.recommendedTasks.forEach((cat) => {
        cat.tasks.forEach((t) => {
          tasks.push({ name: t.name, category: cat.category, points: t.points });
        });
      });
...
function generateHabits(ageGroup: AgeGroup, child: ChildInfo, profile: FamilyProfile) {
  const habits: Array<{ name: string; frequency: string; reason: string }> = [];

  // 基础习惯
  habits.push(
    { name: '按时起床', frequency: 'daily', reason: '培养时间观念' },
    { name: '规律作息', frequency: 'daily', reason: '保证充足睡眠' }
  );

  // 根据年龄添加
  if (ageGroup === 'kindergarten') {
    habits.push(
      { name: '自己穿衣', frequency: 'daily', reason: '培养自理能力' },
      { name: '整理玩具', frequency: 'daily', reason: '养成整洁习惯' }
    );
  } else if (ageGroup === 'primary') {
    habits.push(
      { name: '每日阅读', frequency: 'daily', reason: '培养阅读习惯' },
      { name: '整理书包', frequency: 'daily', reason: '培养条理性' },
      { name: '预习复习', frequency: 'daily', reason: '养成学习习惯' }
    );
  } else {
    habits.push(
      { name: '制定计划', frequency: 'daily', reason: '提高时间管理能力' },
      { name: '错题整理', frequency: 'weekly', reason: '针对性提高' }
    );
  }
...
function generateActivities(child: ChildInfo, profile: FamilyProfile) {
  const activities: Array<{ name: string; type: string; reason: string }> = [];
  const gender = child.gender;

  // 根据性别推荐
  Object.entries(extracurricularActivities).forEach(([type, category]) => {
    // 通用推荐
    category.common.forEach((act) => {
      activities.push({
        name: act.name,
        type,
        reason: `${act.benefit}，适合${act.age}`,
      });
    });

    // 性别特定推荐
    if (gender === 'boy' && 'boyRecommended' in category) {
      category.boyRecommended?.forEach((act) => {
        activities.push({
          name: act.name,
          type,
          reason: `${act.benefit}，适合${act.age}男孩`,
        });
      });
    } else if (gender === 'girl' && 'girlRecommended' in category) {
      category.girlRecommended?.forEach((act) => {
        activities.push({
          name: act.name,
          type,
          reason: `${act.benefit}，适合${act.age}女孩`,
        });
      });
    }
  });
...
function generateReason(
  ...
  return reasons[task.name] || `培养${task.category}能力`;
}
