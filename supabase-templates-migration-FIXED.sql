...
-- 6. 创建访问策略（公开读取，仅管理员可写）
CREATE POLICY IF NOT EXISTS "允许公开读取模板分类" ON template_categories FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "允许公开读取模板使用统计" ON template_usage_stats FOR SELECT USING (true);
...
-- 生活习惯子分类
INSERT INTO template_categories (type, name, name_en, icon, description, parent_id, sort_order) VALUES
('habit', '晨间 routine', 'Morning Routine', '🌤️', '早晨固定习惯', (SELECT id FROM template_categories WHERE type='habit' AND name='生活习惯'), 1),
('habit', '晚间 routine', 'Evening Routine', '🌙', '晚间固定习惯', (SELECT id FROM template_categories WHERE type='habit' AND name='生活习惯'), 2),
...
-- 9. 设置存储策略
CREATE POLICY IF NOT EXISTS "模板图片公开读取" ON storage.objects FOR SELECT USING (bucket_id = 'template-images');
CREATE POLICY IF NOT EXISTS "管理员可上传模板图片" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'template-images');
CREATE POLICY IF NOT EXISTS "管理员可更新模板图片" ON storage.objects FOR UPDATE USING (bucket_id = 'template-images');
