...
- 当前默认皮肤：`forest-comic`
- 当前新增可用皮肤：`arcade-comic`（电玩漫画风，黄黑厚描边游戏化模板）
- 欢迎页默认插画：`public/skins/forest-comic/welcome-comic.svg`
- 电玩漫画风插画：`public/skins/arcade-comic/welcome-comic.svg`
...
`arcade-comic` 以用户提供的黄黑厚描边参考为母版，作为第一套可切换的新 UI 模板：

- **主视觉**：亮黄色主色、黑色粗描边、白/浅灰页面底、右下硬阴影。
- **组件骨架**：顶部栏、底部导航、卡片、按钮、进度条和状态标签都必须保留粗边框与硬投影，不能退回普通柔和卡片风。
- **功能色**：红、蓝、紫、青、绿只用于快捷入口、状态、奖励与惩罚，不作为页面大面积背景。
- **跨端映射**：Web 使用 `[data-theme-skin="arcade-comic"]` CSS 变量；小程序使用 `ui-arcade-*` mixin；Android / iOS 壳层提供同名 arcade token。
- **业务边界**：这是一套皮肤模板，不复制首页、任务页、心愿页、我的页业务逻辑。
